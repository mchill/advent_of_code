...
int travel(queue<int> choices, int last = -1) {
    ...
    for (int i = 0; i < choices.size(); i++) {
        ...
        if (last == -1 && biggest == 0)
            biggest = travel(choices, next);
        else if (last == -1)
            biggest = max(biggest, travel(choices, next));
        else if (biggest == 0)
            biggest = travel(choices, next) + distances[last][next];
        else
            biggest = max(biggest, travel(choices, next) + distances[last][next]);
